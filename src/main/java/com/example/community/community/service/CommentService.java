...
import com.example.community.community.enums.NotificationStatusEnum;
import com.example.community.community.enums.NotificationTypeEnum;
...
import com.example.community.community.mapper.*;
...
public class CommentService {
    ...
    public void insert(Comment comment, User commentator) {
        ...
        if (comment.getType()==CommentTypeEnum.COMMENT.getType()){
            ...
            parentComment.getId(comment.getParentId());
    ...
    public List<CommentDTO> listByTargetId(Long id, CommentTypeEnum type) {
        ...
        commentExample.setOrderByClause("gmt_create desc");//创建时间倒序排序
        List<Comment> comments = commentMapper.selectByExample(commentExample);
        if (comments.size()==0){
            return new ArrayList<>();
        }
        //jsva8语法,使用lambda表达式获取去重的评论人
