...
public class CommentService {
    ...
    public void insert(Comment comment) {
        if (comment.getPrientId()==null||comment.getPrientId()==0){
        ...
        if (comment.getType()==CommentTypeEnum.COMMENT.getType()){
            //回复评论
            Comment dbComment = commentMapper.selectByPrimaryKey(comment.getPrientId());
            if (dbComment==null){
            ...
            //回复问题
            Question question = questionMapper.selectByPrimaryKey(comment.getPrientId());
            if (question==null){
